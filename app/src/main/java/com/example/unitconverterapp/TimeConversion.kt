...
fun  TimeConversion(){
      Column(modifier= Modifier.fillMaxSize(),
            ...
            var inputUnit by remember { mutableStateOf("s") }
            var outputUnit by remember { mutableStateOf("s") }
...
//                  result=BigDecimal(3.14159265359).setScale(2, RoundingMode.HALF_EVEN)
            ...
            Image(painter = painterResource(id =R.drawable.time), contentDescription = "Length",modifier= Modifier
                  .height(150.dp)
                  .width(150.dp)
                  .padding(16.dp))
            Spacer(modifier = Modifier.height(8.dp))

            Text(text = "Time Conversion", style = MaterialTheme.typography.headlineLarge)
                  ...
                  Box(modifier = Modifier)
                  {
                        Button(onClick = { iexpand=true}) {
                              ...
                              DropdownMenu(expanded = iexpand, onDismissRequest = { iexpand=false }) {
                                    ...
                                    DropdownMenuItem(
                                          text = { Text(text = "Milliseconds (ms)") },
                                          onClick = {
                                                inputUnit="ms"
                                    ...
                                    DropdownMenuItem(
                                          text = { Text(text = "Minutes (min)") },
                                          onClick = {
                                                inputUnit="min"
                                                iconversion.value=60.0
                                                iexpand=false
                                                convert()
                                          })
                                    DropdownMenuItem(
                                          text = { Text(text = "Hours (hrs)") },
                                          onClick = {
                                                inputUnit="hrs"
                                                iconversion.value=3600.0
                                                iexpand=false
                                                convert() })
                                    DropdownMenuItem(
                                          text = { Text(text = "Days") },
                                          onClick = {
                                                inputUnit="Days"
                                                iconversion.value=86400.0
                                                iexpand=false
                                                convert()
                                          })
                                    DropdownMenuItem(
                                          text = { Text(text = "Weeks") },
                                          onClick = {
                                                inputUnit="Weeks"
                                                iconversion.value=604800.00
                                                iexpand=false
                                                convert() })
                              }
                        }
